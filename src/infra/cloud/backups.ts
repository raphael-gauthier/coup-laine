...
async function dumpAllTables(): Promise<BackupSnapshot['tables']> {
  return {
    ...
    prestations: await db.select().from(schema.prestations),
...
async function wipeAndRestore(tables: BackupSnapshot['tables']): Promise<void> {
  ...
  await db.delete(schema.prestations);
  ...
  for (const row of tables.prestations) await db.insert(schema.prestations).values(row as typeof schema.prestations.$inferInsert);
...
export async function restoreBackup(name: string): Promise<void> {
  ...
  if (snapshot.schemaVersion !== 1) {
    throw new Error(`Schéma inconnu (v${snapshot.schemaVersion})`);
  }
